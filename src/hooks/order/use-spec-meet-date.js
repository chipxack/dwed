...
export function useSpecMeetDate({activeDay}) {
    ...
    const {$cart: {data}} = useStore($orderModel)
    const [dateRange, setDateRange] = useState([])
    const [meetTimes, setMeetTimes] = useState({})
    const [hours, setHours] = useState([])
    const [requestData, setRequestData] = useState({})
    const operating_modes = data[0]?.responsible?.operating_mode
    ...
    useEffect(() => {
        const day = getWeekDay(new Date(activeDay))
        if (operating_modes && operating_modes[day]) {
            ...
            while (a < to) {
                ...
                possibleMeetTime[parseInt(a)][id] = {
                    ...
                    dateTime: new Date(moment(new Date(`${date.toDateString()} ${moment(getDateTime(a)).format('HH:mm')}`)).format(formatter)).getTime()
                }
                a = parseFloat(strHour.format('HH.mm'))
            }
            setMeetTimes(possibleMeetTime)
            for (let i = from; i < to; i++) {
                const id = new Date(moment(new Date(`${date.toDateString()} 0${i}:59`)).format(formatter)).getTime()
                if (i < 10) {
